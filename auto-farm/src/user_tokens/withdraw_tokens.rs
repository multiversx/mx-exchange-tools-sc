...
pub trait WithdrawTokensModule {
    fn withdraw_specific_tokens(
        ...
        for ttw in tokens_to_withdraw {
            ...
            let mut full_farm_token = all_tokens.get(index);
            require!(
                ttw.amount <= full_farm_token.amount,
                "Not enough balance to withdraw"
            );

            full_farm_token.amount -= ttw.amount;
            if full_farm_token.amount > 0 {
                let _ = all_tokens.set(index, &full_farm_token);
            } else {
