...
use super::farm_config::{FarmConfig, WrappedFarmTokenAttributes, MAX_PERCENT};
    ...
    + multiversx_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn update_farm_after_claim(
        &self,
        initial_farm_config: &FarmConfig<Self::Api>,
        farm_config_mapper: &mut SingleValueMapper<FarmConfig<Self::Api>>,
    ...
    fn compute_user_rewards_payment(
        ...
        let reward = if farm_config.farm_rps > token_rps {
            let rps_diff = &farm_config.farm_rps - &token_rps;
            &payment.amount * &rps_diff / &farm_config.division_safety_constant
        } else {
            BigUint::zero()
        };
        let locked_token_id = self.locked_token_id().get();
        EsdtTokenPayment::new(locked_token_id, farm_config.reward_token_nonce, reward)
    }
